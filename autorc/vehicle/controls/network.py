...
class Network:
    def __init__(self, save_dir):
        self.x_in = tf.placeholder(tf.float32, shape=[None, 15])
        self.exp_y = tf.placeholder(tf.float32, shape=[None, 9, 11])
        self.learning_rate = .5
        self.eval_x = tf.placeholder(tf.float32, shape=[None, 15])
        self.eval_y = tf.placeholder(tf.float32, shape=[None, 9, 11])
        self.build_model()
